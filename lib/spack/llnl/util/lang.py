...
def doublewrap(f):
    ...
    See https://stackoverflow.com/questions/653368.
    """
    @functools.wraps(f)
...
def lazy_lexicographic_ordering(cls, set_hash=True):
    ...
    # these are helpers for lt that evaluate which iterator finished first
    left_shorter  = lambda left, right: left is done
    right_shorter = lambda left, right: right is done

    # lt is implemented much like eq.
    def lt(lseq, rseq, op=operator.lt, diff=left_shorter):
        liter = lseq()
        riter = rseq()
        ...
        for left, right in zip_longest(liter, riter, fillvalue=done):
            ...
            if equal:
                continue
            return (
                lt(left, right, op, diff) if callable(left)
                else op(left, right)
            )
    ...
    def gt(lseq, rseq):
        return lt(lseq, rseq, operator.gt, right_shorter)
